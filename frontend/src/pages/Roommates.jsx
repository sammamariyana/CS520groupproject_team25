...
import {
  ...
  FiCheckCircle, FiX, FiPlusCircle, FiUser, FiLock,
} from 'react-icons/fi'
...
export default function Roommates() {
  ...
  const [authPrompt, setAuthPrompt]     = useState(null) // 'message' | 'save' | 'report'
  ...
  const filtered = roommates.filter(r => {
    ...
    return matchSleep && matchClean
  })

  const handleMessage = id => {
    if (!user) { setAuthPrompt('message'); return }
    setMessageModal(id)
  }
  ...
  const authPromptText = {
    message: 'Log in to send messages to potential roommates.',
    save:    'Log in to save roommate profiles to your dashboard.',
    report:  'Log in to report this profile.',
  }
              ...
              >
                ...
                <div className="flex gap-2 pt-1">
                  {messageSent[r.id] ? (
                    ...
                    <button onClick={() => handleMessage(r.id)}
        ...
        {authPrompt && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              ...
              <p className="text-gray-500 text-sm mb-6">{authPromptText[authPrompt]}</p>
              <div className="flex gap-3">
                <button onClick={() => setAuthPrompt(null)}
                  className="flex-1 border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl text-sm hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
                <button onClick={() => navigate('/login?redirect=/roommates')}
              ...
              <div className="space-y-2 mb-5">
                {['Fake or impersonation account', 'Harassing behavior', 'Spam or scam', 'Inappropriate content', 'Other'].map(reason => (
                  <label key={reason} className="flex items-center gap-3 cursor-pointer p-2.5 rounded-xl hover:bg-gray-50">
                    <input type="radio" name="r2" value={reason} checked={reportReason === reason}
                      onChange={() => setReportReason(reason)} className="accent-blue-600" />
